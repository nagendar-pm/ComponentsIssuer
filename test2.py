...
def time_checker(argumet_str):
	if argumet_str=="expired?":
		with open('json_timestamp.txt','r',encoding='utf-8') as f:
			stored=float(f.read())
			if (time.time()-stored) > update_interval:
				return True
			else:
				return False
	if argumet_str=="just stored":
		with open('json_timestamp.txt','w',encoding='utf-8') as f:
...
def index():
	...
	if request.method == 'POST' and form.validate_on_submit():
		if ('data.json' not in os.listdir()) or ('json_timestamp.txt' not in os.listdir()) or time_checker("expired?"):
			try:
